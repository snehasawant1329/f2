...
class Mystock extends Component {
  ...
  componentDidMount() {
    axios
      .get(`https://financialportfolio-26057.firebaseio.com/stocks.json`)
      .then((db) => {
        const res = db.data;
        if (res === null) {
          this.setState({
          ...
          for (let i = 0; i < Keys.length; i++) {
            ...
            this.StockInfo = (x) => {
              axios
                .get(
                  `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${c_symb}&apikey=R8LUA9SSY4Y3D4KW`
                )
                .then((res) => {
                  let Data = [];
                  let Current_p = [];
